        ...
        init: function (target, vars, tween) {
          ...
          if ((autoRotate = this._autoRotate)) {
            ...
            while (--i > -1) {
              ...
              p = autoRotate[i][2];
              this._initialRotations[i] = this._func[p]
    ...
    function (TweenPlugin, TweenLite) {
          ...
          if (!_specialProps[p]) {
            ...
            _registerComplexSpecialProp(p, {
              parser: function (t, e, p, cssp, pt, plugin, vars) {
                var pluginClass = (_gsScope.GreenSockGlobals || _gsScope).com
      ...
      var _transformProps =
        ...
        _getTransform = (_internals.getTransform = function (
          ...
          if (!s || s === "none" || s === "matrix(1, 0, 0, 1, 0, 0)") {
            //if no transforms are applied, just use the defaults to optimize performance (no need to parse).
            tm = {
              x: 0,
              y: 0,
              z: 0,
          ...
          if (v === 1 || v === 0)
            if (t.force3D === "auto")
              if (
                ...
                //on the final render (which could be 0 for a from tween), if there are no 3D aspects, render in 2D to free up memory and improve performance especially on mobile devices
      ...
      var _removeProp = function (s, p) {
          if (p) {
            if (s.removeProperty) {
              if (p.substr(0, 2) === "ms") {
                //Microsoft browsers don't conform to the standard of capping the first prefix character, so we adjust so that when we prefix the caps with a dash, it's correct (otherwise it'd be "ms-transform" instead of "-ms-transform" for IE9, for example)
                p = "M" + p.substr(1);
              }
...
(function (window, moduleName) {
      ...
      if (this._timeline) {
        ...
        if (!value && elapsed !== 0 && this._initted && this.duration()) {
          this.render(
            ...
            true,
            true
          ); //in case the target's properties changed via some other tween or manual update by the user, we should force a render.
        }
      }
    if (this._gc && !value) {
      this._enabled(true, false);
  ...
  p.render = function (time, suppressEvents, force) {
    ...
    if (time >= duration) {
      ...
      if (
        prevTime !== 0 ||
        (duration === 0 && prevRawPrevTime > 0 && prevRawPrevTime !== _tinyNum)
      ) {
        callback = "onReverseComplete";
        isComplete = this._reversed;
      }
